...
図\ref{fig:d配達員側新規会員登録画面}では、図\ref{fig:dログイン画面}にて新規会員登録のリンクを選択することで表示される画面である。ここでは、まず利用手段について選択してもらう。利用手段に応じて登録する内容が変化する。図\ref{fig:d配達員側新規会員登録画面2}では、入力内容の確認を行う画面である。
...
図\ref{fig:d求人受注画面}では、前述の図\ref{fig:d求人検索詳細画面}と同様の機能を有している画面である。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{./pic/Deliveryer画面デザイン/履修変更画面/履歴書変更確認.pdf}
  \caption{履歴書変更確認画面}
...
\begin{figure}[tbp]
  ...
  \begin{minipage}{.43\linewidth}
    \centering
    \includegraphics[width=\linewidth]{./pic/Deliveryer画面デザイン/口座情報管理画面/口座情報管理画面.pdf}
    \caption{口座情報管理画面}
    \label{fig:d口座情報管理画面}
  \end{minipage}
  \hspace{5mm}
  \begin{minipage}{.43\linewidth}
    \centering
    \includegraphics[width=\linewidth]{./pic/Deliveryer画面デザイン/口座情報管理画面/口座情報変更画面.pdf}
    \caption{口座情報変更画面}
    \label{fig:d口座情報変更画面}
  \end{minipage}
\end{figure}
...
\subsubsection{給与明細}
図\ref{fig:d給与明細画面}では、給与明細が確認できる画面である。ここでは図\ref{fig:給与明細_表示期間の変更}のように表示期間の変更が可能である。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.75\textwidth]{./pic/administrator画面デザイン/111_売り上げ管理画面/111_売り上げ管理画面.pdf}
  \caption{売り上げ管理画面}
  \label{fig:a売り上げ管理画面}
\end{figure}
...
\subsubsection{清算管理画面}
%依頼者の生産管理画面(1121)と生産管理詳細画面(11211)を追加する

\subsubsection{全体通知送信・管理画面}
%全体通知送信・管理画面(113)を追加する
%通知一覧画面(1131)と通知詳細画面(11311)を追加する
%全体通知送信画面(1132)を追加する

\subsubsection{システム設定画面}
%システム設定画面(114)、手数料変更画面(1141)、利用規約更新画面(1143)を追加する

\subsubsection{アカウント管理画面}
%アカウント管理画面(12)を追加する

\subsubsection{依頼者アカウント管理画面}
%依頼者アカウント画面(121)を追加する

\subsubsection{依頼者登録画面}
%依頼者登録画面(1211)を追加する

\subsubsection{依頼者削除画面}
%依頼者削除確認画面(12131)を追加する

\subsubsection{依頼者詳細画面}
%依頼者詳細画面_アカウント情報(12121)、依頼者詳細画面_注文履歴(12122)を追加する
%アカウント情報で見れるのは基本情報、口座情報、問い合わせ履歴、請求履歴
%依頼者情報編集画面(121211)、依頼者詳細画面_注文履歴_注文詳細(121221)を追加する

\subsubsection{配達員アカウント管理画面}
%配達員アカウント画面(122)を追加する

\subsubsection{配達員登録画面}
%依頼者登録画面(1221)を追加する

\subsubsection{配達員削除画面}
%依頼者削除確認画面(12241)を追加する

\subsubsection{配達員申請確認画面}
%配達員申請確認画面(1222)、配達員申請詳細画面(12221)を追加する
%詳細画面には、基本情報、口座情報、履歴書が表示されている

\subsubsection{配達員詳細画面}
%配達員詳細画面_アカウント情報(12231)、配達員詳細画面_配達履歴(12232)を追加する
%アカウント情報で見れるのは基本情報、口座情報、問い合わせ履歴、振込履歴
%配達員情報編集画面(122311)、配達員詳細画面_配達履歴_配達詳細(122321)を追加する
%配達員詳細画面_履歴書確認(12233)、履歴書画面警告通知送信画面(12234)を追加する
...
\subsubsection{店舗削除画面}
%店舗アカウント画面(12341)を追加する

\subsubsection{店舗申請確認画面}
%店舗申請確認画面(1232)、店舗申請詳細画面(12321)を追加する
%詳細画面には、基本情報、口座情報、営業許可証が表示されている

\subsubsection{店舗詳細画面}
%店舗詳細画面_アカウント情報(12331)、店舗詳細画面_販売履歴(12332)を追加する
%アカウント情報で見れるのは基本情報、口座情報、問い合わせ履歴、振込履歴
%店舗情報編集画面(123311)、店舗詳細画面_販売履歴_販売詳細(123321)を追加する
%店舗詳細画面_営業許可証(12333)を追加する

\subsubsection{管理者アカウント管理画面}
%管理者アカウント画面(124)を追加する

\subsubsection{管理者登録画面}
%管理者登録画面(1241)を追加する

\subsubsection{管理者削除画面}
%管理者削除確認画面(12431)を追加する

\subsubsection{管理者詳細画面}
%管理者詳細画面_アカウント情報(12421)、管理者詳細画面_管理者権限(12422)を追加する
%アカウント情報で見れるのは基本情報のみ
%管理者情報編集画面(124211)、管理者権限編集画面(124221)を追加する

\subsubsection{問い合わせ対応画面}
%問い合わせ対応画面(13)を追加する

\subsubsection{問い合わせ詳細画面}
%問い合わせ詳細画面(131)、問い合わせ対応メール送信・編集(1311)を追加する

\subsubsection{誤配達対応画面}
%誤配達対応画面(132)、配達詳細(1321)を追加する

\subsubsection{通報管理画面}
%通報管理画面(133)、通報内容詳細画面(1331)を追加する

\subsubsection{問い合わせ例編集画面}
%問い合わせ例確認(1341)、問い合わせ例詳細確認(1342)、問い合わせ例編集(13411)、問い合わせ例詳細編集(13421)を追加する
